...
play_game_set = set([u'CAUSE-BINARY-OUTCOME', u'ATTEMPT-AGENT',
                     u'CAUSE-ATTEMPT-POSITIVE-OUTCOME'])
...
no_agent_play_game_set = set([u'CAUSE-ATTEMPT-POSITIVE-OUTCOME',
                              u'CAUSE-BINARY-OUTCOME'])
eradicate_set = set([u'THING-NOT-EXISTING', u'CAUSE-NOT-EXIST',
                     u'THING-CAUSING-NOT-EXIST'])
no_agent_eradicate_set = set([u'THING-NOT-EXISTING', u'CAUSE-NOT-EXIST'])
taste_set = set([u'THING-BEING-EXPERIENCED', u'CAUSE-EXPERIENCE',
                 u'THING-CAUSING-EXPERIENCE'])
no_agent_taste_set = set([u'THING-BEING-EXPERIENCED', u'CAUSE-EXPERIENCE'])
exit_abyss_set = set([u'THING-THAT-LIMITED-OPTIONS', u'CAUSE-INCREASE-OPTIONS',
                      u'THING-THAT-STOPPED-FUNCTION',
                      u'CAUSE-RESUME-FUNCTION'])
deep_abyss_set = set([u'CAUSE-SEVERE-NOT-FUNCTION',
                      u'CAUSE-SEVERE-REDUCE-OPTIONS'])
harvest_crop_seed_set = set([u'CAUSE-OUTCOME-OF-ACTION',
                             u'CAUSE-REALIZE-OUTCOME',
                             u'PREPARATION-FOR-OUTCOME', u'THING-REALIZING',
                             u'OUTCOME-OF-ACTION'])
...
addiction_set = set([u'CAUSE-NOT-FUNCTION', 'CAUSE-BAD-JUDGEMENT',
                     'CAUSE-OBSESSION'])
...
protect_set = set([u'CAUSE-ALLOW-FUNCTION', u'THING-ALLOWING',
                   u'THING-FUNCTIONING'])
protection_set = set([u'CAUSE-ALLOW-FUNCTION'])
cost_set = set([u'CAUSE-DRAIN-RESOURCES', u'THING-DRAINING'])
price_set = set([u'THING-DESIRED',
                 u'CAUSE-NEGATIVE-CONSEQUENCE-OF-DESIRED-THING'])
no_desire_price_set = set([u'CAUSE-NEGATIVE-CONSEQUENCE-OF-DESIRED-THING'])
pay_set = set([u'THING-DESIRED', u'CAUSE-NEGATIVE-CONSEQUENCE-OF-DESIRED-THING',
               'CAUSE-EXCHANGE-NEGATIVE-POSITIVE'])
live_in_set = set([u'CAUSE-EXPERIENCE-SOMETHING', u'EXPERIENCER',
                   'THING-EXPERIENCED'])
...
def process_explanation(exp, s_id, lang, target_sub, target_lms, source_lms):
    ...
    if logic_set == game_set:
        ...
        if lang == 'EN':
            ...
            print lms['cause-binary-outcome'] + \
              u' означает, что есть два возможных исхода: ' + \
              u'положительный и отрицательный, связанные с идеей' + \
              lms['game-stakes']
    ...
    if logic_set == no_stakes_game_set:
        ...
        if lang == 'EN':
            ...
            print lms['cause-binary-outcome'] + \
              u' означает, что есть два возможных исхода: ' + \
              u'положительный и отрицательный'
    ...
    if logic_set == play_game_set:
        ...
        if lang == 'EN':
            ...
            print lms['cause-binary-outcome'] + \
              u' означает, что есть два возможных исхода: положительный ' + \
              u'и отрицательный; ' + lms['cause-attempt-outcome'] + \
              u' означает, что ' + lms['agent-attempt'] + \
              u' старается достичь положительного исхода'
    ...
    if logic_set == play_set:
        ...
        if lang == 'EN':
            ...
            print lms['cause-attempt-outcome'] + u' означает, что ' + \
              lms['agent-attempt'] + \
              u' старается достичь положительного исхода'
    ...
    for jline in data:
        ...
        if len(mappings["explanation"])>0:
            ...
            process_explanation(exp_list, sent_id, lang, target_sub,
                                target_lms, source_lms)


def parse_arguments():
    parser = argparse.ArgumentParser(description='Generate natural language '
                                     'from metaphor system output.')
