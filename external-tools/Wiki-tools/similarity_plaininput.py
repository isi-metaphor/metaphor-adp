...
def similarity(results, tword, tpos, method, output, lang):
    ...
    for i in xrange(len(results)):
        ...
        if method == 'path_similarity':
            ...
            if (tpos not in POSMAP) or POSMAP[tpos] != results[i][1]:
                similarity = 0.0
            else:
            ...
            similarity = sw.path_similarity(tword, tpos, results[i][0],
                                            results[i][1])
        elif method == 'conceptnet_path':
            similarity = sc.conceptnet_similarity(tword, tpos, results[i][0],
                                                  results[i][1], lang)
        elif method == 'translation_path':
            similarity = st.translation_similarity(tword, tpos, results[i][0],
                                                   results[i][1], lang)
        if not similarity:
            similarity = 0.0
    ...
    with open(output, 'w') as outputFile:
        if lang == 'EN':
            ...
            similarity(results, tword, tpos, 'wup_similarity', outputFile,
                       lang)
            similarity(results, tword, tpos, 'wordnet_drf_path', outputFile,
                       lang)
        else:
            # the following only support for RU, ES, FA
            similarity(results, tword, tpos, 'conceptnet_path', outputFile,
                       lang)
            similarity(results, tword, tpos, 'translation_path', outputFile,
                       lang)
