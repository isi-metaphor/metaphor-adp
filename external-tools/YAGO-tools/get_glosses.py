...
def main():
    ...
    if preferred_meaning:
        query = query.replace("yf2.predicate='rdfs:label'",
                              "yf2.predicate='<isPreferredMeaningOf>'")
    ...
    try:
        ...
        for row in rows:
            ...
            # dict with key=category; value=gloss; I believe that there is
            # only one gloss/category
