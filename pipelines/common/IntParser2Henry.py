...
# This is used for Spanish, Farsi, and Russian.
...
def main():
    parser = argparse.ArgumentParser(
        description='Convert logical forms into Henry input, adding '
        'nonmerge contraints.')
    ...
    output_str = ''
    id_prop_args_pattern = re.compile(r'(\[(\d+)\]:)?([^\[\(]+)(\((.+)\))?')
    sent_id_pattern = re.compile(r'id\((.+)\)')
    text_id_pattern = re.compile(r'% TEXTID\s*\(\s*([^\s\t]+)')
    postfix_pattern = re.compile(r'.+-([a-z]+)$')
    sent_id = ''
    prop_id_counter = 0
    text_id = ''
    just_TEXT_ID = 0
        ...
        if line.startswith('%'):
            ...
            for prop in props:
                ...
                if matchObj:
                    ...
                    if matchObj.group(2):
                        word_id = matchObj.group(2)
                    else:
                        word_id = 'ID' + str(prop_id_counter)
                    ...
                    if samepred:
                        ofile.write(nm)
                # else:
                #     print 'Strange proposition: ' + prop + '\n'
