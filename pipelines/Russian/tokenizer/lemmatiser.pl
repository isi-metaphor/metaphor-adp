...
# The script does lemmatisation of the output of Treetagger or TnT using
# cstlemmatiser for unknow words
...
# a known pos that can do no harm (cst lemmatises everything and
        ...
        foreach (split /\t/,substr($_,$tabpos)) {
            if (($posgr,$l)=/^([A-Z0-9-]+?) (.+)/i) {
                ...
                if (exists $lex{$w}{$posgr}) {
                    $oldlex=$lex{$w}{$posgr};
                    unless ($oldlex eq $l) {
                        if (length($l)>length($oldlex)) {
                            # when merging two form-pos pairs, we keep the
                            # longer lemma
                            $lex{$w}{$posgr}=$l;
                        }
                    }
