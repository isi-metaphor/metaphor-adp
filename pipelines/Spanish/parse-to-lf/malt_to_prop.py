    ...
    for line in infile:
        ...
        if len(line) > 1:
            ...
            if date.match(wordLemma):
                ...
                wordLemma = wordText
            info = [longID, wordText, wordLemma, wordPOS, wordHead, wordRel,
                    wordID, sent_count]
...
def prop_to_dict(props, eCount, xCount, uCount):
    ...
    for prop in props:
        ...
        if propTags.match(pos):
            args, tag, eCount, xCount, uCount, question \
                = build_predicate(pos, eCount, xCount, uCount, lemma,
                                  question, token)
            new_prop.extend([token, lemma, pos, head, rel, shortID, args,
                             tag, propID])
            sent_dict[shortID] = [token, lemma, pos, head, rel, shortID, args,
                                  tag, propID]
...
def replace_args(prop_sent, sent_dict):
    ...
    for prop in prop_sent:
        ...
        if ((rel == "suj") or (rel == "spec")) and (tag != "NULL") and \
           (realHead(sent_dict, head)):
            sent_dict = insert_subj(head, wordID, sent_dict)
        ...
        if (tag == "vb" and rel == "v" and lemma in passivesList) and \
           realHead(sent_dict, head) and \
           int(sent_dict[head][8]) == int(sent_dict[wordID][8]) + 1:
            sent_dict = process_passive(head, wordID, sent_dict)
        if rel == "v" and lemma not in passivesList and tag != "" and \
           realHead(sent_dict, head):
        ...
        if ((rel in adjectiveRels and (pos == "a")) or
                (lemma in quantifierList)) and realHead(sent_dict, head):
            sent_dict = insert_adjHead(head, wordID, sent_dict)
        if rel == "cc" and pos == "r" and lemma not in whWords and \
           realHead(sent_dict, head):
        ...
        if lemma in subConList and realHead(sent_dict, head) and \
           len(predicate) > 2:
...
def handle_wh(head, wordID, sent_dict):
    ...
    if sent_dict[head][7] == "vb" or sent_dict[head][7] == "in":
        ...
        if headHead == 0:
            ...
            if sent_dict[headHead][7] == "vb" or \
               sent_dict[headHead][7] == "in":
...
def insert_subj(head, wordID, sent_dict):
    """Insert the subject of a verb as its first argument"""
    if sent_dict[wordID][4] == "spec" and \
       sent_dict[wordID][1] not in thingProList:
        return sent_dict
    ...
    elif (sent_dict[head][7] == "vb" and sent_dict[wordID][7] != "nn" and
          sent_dict[wordID][6][0] != "R"):
...
def inherit_args(head, wordID, sent_dict):
    ...
    if sent_dict[head][7] == "vb" and \
       not nounArg.search(sent_dict[wordID][6][1]):
        ...
        # if not nounArg.search(sent_dict[wordID][6][2]) and \
        #    nounArg.search(sent_dict[head][6][2]):
    ...
    #                 and values[1] in andOrList):
    #             conjHead = values[3]
    #             headHead = sent_dict[conjHead][3]
    #             if headHead != 0 and sent_dict[headHead][7]:
    #                 sent_dict = add_new_verb(
    #                     sent_dict, sent_dict[headHead][1],
    #                     sent_dict[headHead][7], sent_dict[headHead][6],
    #                     sent_dict[wordID][6][0], sent_dict[headHead][8],
    #                     sent_dict[head][8], wordID, head)
    #             # If the conjuction is "o" add an "or" proposition
    #             if (values[1] == "o"):
    #                 sent_dict, newKey = add_new_entry(
    #                     sent_dict, "or", sent_dict[head][6][0],
    #                     sent_dict[wordID][6][0], sent_dict[wordID][8])
    return sent_dict
    ...
    if sent_dict[head][1] in copulaList:
        ...
        if sent_dict[wordID][7] == "nn":
            ...
            sent_dict, newKey = add_new_entry(
                ...
                sent_dict[wordID][6][1], sent_dict[wordID][8])
            sent_dict = replace_old_args(
                sent_dict, sent_dict[head][6][0], newKey)
...
def add_new_verb(sent_dict, token, tag, hhargs, arg2, verbID, currID, wordID,
                 head):
    ...
    if sent_dict[headHead][1] == "ser":
        sent_dict, newKey = add_new_entry(
            sent_dict, "equal", arg1, arg2, sent_dict[wordID][8])
    else:
...
def insert_cd(head, wordID, sent_dict):
    ...
    if sent_dict[wordID][2] == "v" and sent_dict[head][7] == "vb" and \
       not nounArg.search(sent_dict[head][6][2]) and \
       not nounArg.search(sent_dict[wordID][6][1]):
...
def insert_subCon_head(head, wordID, sent_dict):
    ...
    if sent_dict[head][7] == "vb" and sent_dict[head][4] not in rootList:
...
def insert_sn(head, wordID, sent_dict):
    ...
    if sent_dict[head][7] == "in" and sent_dict[wordID][7] != "nn":
        ...
        sent_dict[head][6][2] = sent_dict[wordID][6][1]
    elif (sent_dict[head][7] == "nn" and sent_dict[wordID][7] == "nn") and \
         (int(sent_dict[head][8]) == int(sent_dict[wordID][8]) + 1):
        if int(sent_dict[head][6][1].split("x")[1]) == \
           int(sent_dict[wordID][6][1].split("x")[1]) - 1:
            sent_dict[wordID][6][1] = sent_dict[head][6][1]
        for key, values in sent_dict.items():
            # Look for a conjunction with the same head as the current word
            if (values[4] == "conj") and (values[3] == sent_dict[wordID][3]):
                ...
                if realHead(sent_dict, headHead):  # != 0:
                    sent_dict = add_new_verb(
                        sent_dict, sent_dict[headHead][1],
                        sent_dict[headHead][7], sent_dict[headHead][6],
                        sent_dict[wordID][6][1], sent_dict[headHead][8],
                        sent_dict[head][8], wordID, head)
...
heProList = ["el", "lo"]
sheProList = ["ella", "la"]
personProList = ["yo", "me", "nos", "nosotros", "usted", "ustedes", "mi",
                 "mis", "su", "sus", "suyo", "nuestro", "nuestros", "nuestra",
                 "nuestras", "quién", "tu", "tú", "quien", "tus", "mío"]
thingProList = ["ellos", "ellas", "él", "este", "ese", "suyo", "aquel"]
reflexProList = ["se"]
possessiveProList = ["mi", "mis", "tu", "tus", "su", "sus", "nuestro",
                     "nuestros", "nuestra", "nuestras", "suyo", "mío"]
...
insertList = ["equal", "card", "or", "be", "loc", "manner", "time", "nn",
              "reason", "of-in", "person"]
inheritingVbs = ["S", "v"]
...
whWords = ["dónde", "cómo", "donde", "cuando", "cuándo", "por_qué", "quién",
           "Quién"]
subConList = ["porque", "mientras_que", "siempre_que", "puesto_que",
              "ya_que", "pues"]
    ...
    for sent, words in zip(full_sents, all_words):
        ...
        if metaFound and not sentIDre.search(sent[0]) and (sent != ['.']):
            try:
                ...
                prop_sent, prop_dict, eCount, xCount, uCount \
                    = prop_to_dict(words, eCount, xCount, uCount)
        ...
        if (not sentIDre.search(sent[0])) and (not metaFound) and \
           (sent != ['.']):
            sent_count += 1
            print "% " + " ".join(sent)
