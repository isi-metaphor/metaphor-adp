...
def output_axiom_freq(input,freqs):
    with open(input, 'r') as openfile:
        lines = filter(lambda l: l.startswith('('),
                       (line.rstrip() for line in openfile))
        for line in lines:
            rs = find_right_side(line)
            # The following 'if' loop processes only single-token axioms.
            # We could add a second conditional for > 2 right side split
...
def main():
    ...
    parser.add_argument('-f', '--frequencies',
                        help='File containing frequencies. Default is English '
                        'frequency file in this directory.',
